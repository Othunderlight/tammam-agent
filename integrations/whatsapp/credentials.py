...
async def get_credentials_from_django(whatsapp_user_id: str) -> dict:
    """Fetch credentials from Django based on WhatsApp user ID."""
    url = f"{SYSTEM_API_ENDPOINT}/integrations/credentials/"
    headers = {
        "Authorization": f"Api-Key {SYSTEM_API_KEY}",
        "Content-Type": "application/json",
    }
    data = '{"key_value": "' + str(whatsapp_user_id) + '"}'
